...
// Энергонезависимая память (NVS) для хранения маршрутов
Preferences prefs;
...
void handleSetRoutes() {
  if (server.hasArg("val")) {
    ...
    // --- СОХРАНЯЕМ МАРШРУТЫ В ЭНЕРГОНЕЗАВИСИМУЮ ПАМЯТЬ (NVS) ---
    prefs.begin("routes", false); // false = режим записи
    for (int i = 0; i < 5; i++) {
      prefs.putString(("r" + String(i)).c_str(), trackRoutes[i]);
    }
    prefs.end();
...
void handleGetStatusData() {
  ...
  // --- ОТДАЁМ ТЕКУЩИЕ МАРШРУТЫ ИЗ ПАМЯТИ ---
  json += ",\"routes\":[";
  for (int i = 0; i < 5; i++) {
    json += "\"" + trackRoutes[i] + "\""; // в кавычках, чтобы "02" не потеряло ноль
    if (i < 4) json += ",";
  }
  json += "]";
...
// ========== SETUP & LOOP ==========
void setup() {
  // --- ЗАГРУЖАЕМ СОХРАНЁННЫЕ МАРШРУТЫ ИЗ NVS (если есть) ---
  prefs.begin("routes", true); // true = только чтение
  for (int i = 0; i < 5; i++) {
    trackRoutes[i] = prefs.getString(("r" + String(i)).c_str(), trackRoutes[i]); // нет ключа — останется дефолт
  }
  prefs.end();
...
<script>
    let routesLoaded = false; // маршруты из памяти подтягиваем один раз при загрузке
    ...
    function updateDashboard() {
        fetch('/get_status_data')
            .then(res => res.json())
            .then(data => {
                // Подтянуть маршруты из памяти ESP32 (один раз при открытии страницы)
                if (!routesLoaded && data.routes) {
                    for (let i = 1; i <= 5; i++) {
                        document.getElementById('route_' + i).value = data.routes[i-1];
                    }
                    routesLoaded = true;
                }
    ...
    updateDashboard(); // первый опрос сразу, не дожидаясь таймера
